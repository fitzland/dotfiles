...
source_pywal() {
	...
	WALLDIR="`xdg-user-dir PICTURES`/wallpapers/archcraft"
...
## Wallpaper ---------------------------------
apply_wallpaper() {
	sed -i -e "s|output DP-2 bg.*|output DP-2 bg $wallpaper1 fill|g" ${DIR}/sway-output
	sed -i -e "s|output DP-3 bg.*|output DP-3 bg $wallpaper2 fill|g" ${DIR}/sway-output
}
